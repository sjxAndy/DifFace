...
# configs['model']['params']['beta_schedule']['val']['n_timestep'] = 200
...
set_new_noise_schedule(model, configs['model']['params']['beta_schedule']['train'], schedule_phase='train')

util_net.reload_model(model, ckpt['state_dict'])
# set_new_noise_schedule(model, configs['model']['params']['beta_schedule']['val'], schedule_phase='val')
model.set_ddim(configs['model']['params']['beta_schedule']['val']['n_timestep'])
...
with torch.no_grad():
    ...
    HQ = model.ddim_sample(val_data['LQ'], False)
...
lq_img = tensor2img(visuals['LQ'], rgb2bgr=True, out_type = np.float32)
hq_img = tensor2img(visuals['HQ'], rgb2bgr=True, out_type = np.float32, min_max=(-1, 1))
# hq_img = tensor2img(visuals['HQ'], rgb2bgr=True, out_type = np.float32)
gt_img = tensor2img(visuals['GT'], rgb2bgr=True, out_type = np.float32)

cv2.imwrite('tmp/HQ_nor_100.png', hq_img * 255)
cv2.imwrite('tmp/LQ_nor.png', lq_img * 255)
cv2.imwrite('tmp/GT_nor.png', gt_img * 255)
